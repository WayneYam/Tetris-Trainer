...
void motion_register(Keybind t, int s, Board &B){
    if(s == 1){
        if(t == Keybind::move_left) B.move_piece(2), status[2].second.restart();
        if(t == Keybind::soft_drop) B.move_piece(3);
        if(t == Keybind::move_right) B.move_piece(0), status[0].second.restart();
    }
    status[(int)t].first = s;
    if(s == 1) status[(int)t].second.restart();
}

void do_motion(Board &B){ // there's actually no ARR and SDD support yet :(
